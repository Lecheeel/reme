...
class QuestionBanks extends Table {
  @override
  String get tableName => 'question_banks';

  TextColumn get id => text()();
  TextColumn get name => text()();
  TextColumn get source => text()();
  TextColumn get subject => text().withDefault(const Constant(''))();
  IntColumn get builtIn =>
      integer().named('built_in').withDefault(const Constant(0))();
  IntColumn get importedAt => integer().named('imported_at').nullable()();
...
class DailyStats extends Table {
  ...
  IntColumn get newCount =>
      integer().named('new_count').withDefault(const Constant(0))();
  IntColumn get reviewCount =>
      integer().named('review_count').withDefault(const Constant(0))();
  IntColumn get correctCount =>
      integer().named('correct_count').withDefault(const Constant(0))();
  IntColumn get graduatedCount =>
      integer().named('graduated_count').withDefault(const Constant(0))();
  IntColumn get checkedIn =>
      integer().named('checked_in').withDefault(const Constant(0))();
...
class ReviewEvents extends Table {
  ...
  TextColumn get eventKind =>
      text().named('event_kind').withDefault(const Constant('rating'))();
...
class DatabaseHelper extends _$DatabaseHelper {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (m, from, to) async {
      if (from < 2) {
        await m.createTable(questionBanks);
        await m.addColumn(questions, questions.bankId);
      }
    },
  ...
  Future<void> _createIndexes() async {
    await customStatement(
      'CREATE INDEX IF NOT EXISTS cards_due_active_idx ON cards(due, suspended)',
    );
    await customStatement(
      'CREATE INDEX IF NOT EXISTS questions_chapter_idx ON questions(chapter)',
    );
    await customStatement(
      'CREATE INDEX IF NOT EXISTS review_events_card_time_idx ON review_events(card_id, reviewed_at)',
    );
    await customStatement(
      'CREATE INDEX IF NOT EXISTS review_events_time_idx ON review_events(reviewed_at, voided)',
    );
  }
  ...
  Future<void> seedIfNeeded() async {
    await ensureBuiltInBank();
    final loader = QuestionBankLoader();
    for (final assetPath in QuestionBankLoader.chapterFiles) {
      final (chapterId, chapterName, version) = await loader.loadChapterMeta(
        assetPath,
      );
      ...
      await transaction(() async {
        await _exec('DELETE FROM questions WHERE bank_id = ? AND chapter = ?', [
          Variable.withString('builtin_politics'),
          Variable.withString(chapterName),
        ]);
  ...
  Future<QuestionBankInfo> ensureBuiltInBank() async {
    await _exec(
      "INSERT OR IGNORE INTO question_banks (id, name, source, subject, built_in) VALUES ('builtin_politics', '考研政治基础题库', 'builtin', '考研政治', 1)",
      [],
    );
    return (await getQuestionBanks()).firstWhere(
      (b) => b.id == 'builtin_politics',
    );
  }

  Future<QuestionBankInfo> createQuestionBank({
    required String id,
    ...
    await _exec(
      'INSERT INTO question_banks (id, name, source, subject, built_in, imported_at) VALUES (?, ?, ?, ?, 0, ?)',
      [
    ...
    return (await getQuestionBanks()).firstWhere((b) => b.id == id);
  }

  Future<QuestionBankInfo> importQuestionBank({
    required String id,
    required String name,
    required String source,
    required String subject,
    ...
    await _insertQuestions(questions, bankId: id);
    return (await getQuestionBanks()).firstWhere((b) => b.id == id);
  });

  Future<List<QuestionBankInfo>> getQuestionBanks() async {
    final rows = await customSelect(
      'SELECT b.*, COUNT(q.id) AS question_count FROM question_banks b LEFT JOIN questions q ON q.bank_id = b.id GROUP BY b.id ORDER BY b.built_in DESC, b.imported_at DESC, b.name',
    ).get();
    return rows
        .map(
          (r) => QuestionBankInfo(
            ...
            builtIn: _int(r, 'built_in') == 1,
            questionCount: _int(r, 'question_count'),
          ),
        )
        .toList();
  }
  ...
  Future<String> getActiveBankId() async {
    final rows = await customSelect(
      ...
      variables: [Variable.withString('active_bank_id')],
    ).get();
    return rows.isEmpty ? 'builtin_politics' : rows.first.read<String>('value');
  }

  Future<void> setActiveBank(String bankId) => _exec(
    'INSERT INTO meta (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value',
    [Variable.withString('active_bank_id'), Variable.withString(bankId)],
  );

  Future<void> deleteQuestionBank(String bankId) => transaction(() async {
    if (bankId == 'builtin_politics') throw ArgumentError('内置题库不可删除');
    await _exec(
      'DELETE FROM cards WHERE question_id IN (SELECT id FROM questions WHERE bank_id = ?)',
      [Variable.withString(bankId)],
    );
    await _exec(
      'DELETE FROM review_events WHERE card_id IN (SELECT id FROM questions WHERE bank_id = ?)',
      [Variable.withString(bankId)],
    );
    await _exec('DELETE FROM questions WHERE bank_id = ?', [
      Variable.withString(bankId),
    ]);
    await _exec('DELETE FROM question_banks WHERE id = ?', [
      Variable.withString(bankId),
    ]);
    if (await getActiveBankId() == bankId) {
      await setActiveBank('builtin_politics');
    }
  });
  ...
  Future<void> _setSeedVersion(String chapterId, int version) => _exec(
    'INSERT INTO meta (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value',
    [
      Variable.withString('seed_version:$chapterId'),
      Variable.withString('$version'),
    ],
  );

  Future<void> insertQuestions(List<Question> questions, {String? bankId}) =>
      transaction(() => _insertQuestions(questions, bankId: bankId));

  Future<void> _insertQuestions(
    ...
    for (final q in questions) {
      final resolvedBankId =
          bankId ??
          (q.id.startsWith('anki:')
              ? q.id.split(':').take(2).join(':')
              : 'builtin_politics');
      await _exec('''
        INSERT OR IGNORE INTO questions
          (id, bank_id, subject, chapter, knowledge_point, knowledge_point_id, type, stem, options, answer, explanation)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ''', _questionVariables(q, bankId: resolvedBankId));
    }
  }

  List<Variable<Object>> _questionVariables(Question q, {String? bankId}) => [
    Variable.withString(q.id),
    Variable.withString(bankId ?? 'builtin_politics'),
  ...
  Future<List<Question>> getDueQuestions({
    String? chapter,
    List<String>? relatedKpIds,
  }) async {
    final now = DateTime.now().millisecondsSinceEpoch;
    final activeBankId = await getActiveBankId();
    final filters = <String>[
      '(c.question_id IS NULL OR (c.due <= ? AND c.suspended = 0))',
    ];
    final vars = <Variable<Object>>[
      Variable.withString(activeBankId),
    ...
    if (chapter != null || related.isNotEmpty) {
      ...
      if (related.isNotEmpty) {
        parts.add(
          'q.knowledge_point_id IN (${List.filled(related.length, '?').join(',')})',
        );
    ...
    final rows = await customSelect('''
      ...
      WHERE q.bank_id = ? AND (c.question_id IS NULL OR (c.due <= ? AND c.suspended = 0))
  ...
  Future<ReviewWriteResult> recordReview({
    ...
    final elapsed = before.lastReview == null
        ? 0.0
        : reviewedAt.difference(before.lastReview!).inMilliseconds /
    ...
    return transaction(() async {
      ...
      final eventId = (await customSelect(
  ...
  Future<void> undoReview({
    ...
    await _exec(
      'UPDATE review_events SET voided = 1 WHERE id = ? AND voided = 0',
  ...
  Future<String> _eventCardId(int eventId) async {
    final row = await customSelect(
      'SELECT card_id FROM review_events WHERE id = ?',
  ...
  Future<void> setSuspended(String questionId, bool value) async {
    ...
    if (existing == null && value) {
      await _exec('INSERT INTO cards (question_id, suspended) VALUES (?, 1)', [
        Variable.withString(questionId),
      ]);
    } else if (existing != null && !value && existing.reps == 0) {
      await deleteCard(questionId);
    } else if (existing != null) {
      await _exec('UPDATE cards SET suspended = ? WHERE question_id = ?', [
        Variable.withInt(value ? 1 : 0),
        Variable.withString(questionId),
      ]);
    }
  }

  Future<void> deleteCard(String questionId) => _exec(
    'DELETE FROM cards WHERE question_id = ?',
    [Variable.withString(questionId)],
  );

  Future<(int, int, int)> getQueueCounts() async {
    final now = DateTime.now().millisecondsSinceEpoch;
    final bankId = await getActiveBankId();
    final row = await customSelect(
      '''
      SELECT
  ...
  Future<List<Question>> getWeakQuestions() async {
    ...
    final rows = await customSelect(
      ...
      WHERE q.bank_id = ? AND c.lapses >= 2 AND c.due <= ? AND c.suspended = 0 ORDER BY c.due ASC
    ''',
      variables: [
        Variable.withString(bankId),
        Variable.withInt(DateTime.now().millisecondsSinceEpoch),
  ...
  Future<double?> getAvgRepsForMastered() async {
    final bankId = await getActiveBankId();
    final row = await customSelect(
      'SELECT AVG(c.reps) AS avg_reps FROM cards c INNER JOIN questions q ON q.id = c.question_id WHERE q.bank_id = ? AND c.reps > 0 AND c.stability >= ?',
      variables: [
        Variable.withString(bankId),
        Variable.withReal(masteredStabilityThreshold),
      ],
  ...
  Future<void> upsertDailyStat(DailyStat stat) =>
      _applyDailyDelta(stat, add: true);
  ...
  Future<List<DailyStat>> getDailyStats(int days) async {
    ...
    final rows = await customSelect(
      ...
      variables: [Variable.withString(dateStr(start))],
    ).get();
    final map = {
      for (final r in rows) r.read<String>('date'): _dailyFromRow(r),
    };
    return [
  ...
  Future<DailyStat?> getTodayStat() async {
    final rows = await customSelect(
      ...
      variables: [Variable.withString(dateStr(DateTime.now()))],
    ).get();
    return rows.isEmpty ? null : _dailyFromRow(rows.first);
  }

  Future<void> checkIn(String date) => _exec(
  ...
  Future<int> getCheckInStreak() async {
    final rows = await customSelect(
      'SELECT date FROM daily_stats WHERE checked_in = 1 ORDER BY date DESC',
    ).get();
    final checked = {for (final r in rows) r.read<String>('date')};
    var day = DateTime.now();
    if (!checked.contains(dateStr(day))) {
  ...
  Future<List<Question>> getReviewQuestions({
    ...
    final activeBankId = bankId ?? await getActiveBankId();
    final related = relatedKpIds ?? const <String>[];
    final parts = <String>[];
    final vars = <Variable<Object>>[
      Variable.withString(activeBankId),
    ...
    final rows = await customSelect('''
      SELECT q.* FROM questions q INNER JOIN cards c ON q.id = c.question_id
      WHERE q.bank_id = ? AND c.reps > 0 AND c.due <= ? AND c.suspended = 0$scope ORDER BY c.due ASC
    ''', variables: vars).get();
    return rows.map(_questionFromRow).toList();
  }
  ...
  Future<List<Question>> getNewQuestions({
    ...
    final activeBankId = bankId ?? await getActiveBankId();
    final vars = <Variable<Object>>[Variable.withString(activeBankId)];
    var scope = '';
    if (chapter != null) {
      scope = ' AND q.chapter = ?';
      vars.add(Variable.withString(chapter));
    ...
    final rows = await customSelect('''
      SELECT q.* FROM questions q LEFT JOIN cards c ON q.id = c.question_id
      WHERE q.bank_id = ? AND (c.question_id IS NULL OR c.reps = 0) AND COALESCE(c.suspended, 0) = 0$scope
      ORDER BY q.id LIMIT ?
    ''', variables: vars).get();
    return rows.map(_questionFromRow).toList();
  }
  ...
  Future<List<int>> getDueDistribution(int days) async {
    ...
    final bankId = await getActiveBankId();
    final rows = await customSelect(
      'SELECT c.due FROM cards c INNER JOIN questions q ON q.id = c.question_id WHERE q.bank_id = ? AND c.reps > 0 AND c.due > ? AND c.due <= ? AND c.suspended = 0',
      variables: [
        Variable.withString(bankId),
        Variable.withInt(now.millisecondsSinceEpoch),
        Variable.withInt(now.add(Duration(days: days)).millisecondsSinceEpoch),
      ],
    ).get();
    final buckets = List<int>.filled(days + 1, 0);
    for (final row in rows) {
      final index =
          ((row.read<int>('due') - now.millisecondsSinceEpoch) /
                  Duration.millisecondsPerDay)
              .floor()
              .clamp(0, days);
      buckets[index]++;
  ...
  DailyStat _dailyFromRow(QueryRow r) => DailyStat(
    ...
    newCount: _int(r, 'new_count'),
    reviewCount: _int(r, 'review_count'),
    correctCount: _int(r, 'correct_count'),
    graduatedCount: _int(r, 'graduated_count'),
    checkedIn: _int(r, 'checked_in') == 1,
    checkInTime: r.readNullable<int>('check_in_time') == null
        ? null
        : DateTime.fromMillisecondsSinceEpoch(r.read<int>('check_in_time')),
  );

  Question _questionFromRow(QueryRow row) => Question(
    id: row.read<String>('id'),
    subject: row.read<String>('subject'),
    chapter: row.read<String>('chapter'),
    knowledgePoint: row.read<String>('knowledge_point'),
    knowledgePointId: row.readNullable<String>('knowledge_point_id') ?? '',
    type: QuestionType.fromName(row.read<String>('type')),
    stem: row.read<String>('stem'),
    options: (jsonDecode(row.read<String>('options')) as List)
        .map((e) => QuestionOption.fromJson(e as Map<String, dynamic>))
        .toList(),
    answer: (jsonDecode(row.read<String>('answer')) as List)
        .map((e) => e as String)
        .toList(),
  ...
  CardState _cardFromRow(QueryRow row) => CardState(
    ...
    due: row.readNullable<int>('due') == null
        ? null
        : DateTime.fromMillisecondsSinceEpoch(row.read<int>('due')),
    lastReview: row.readNullable<int>('last_review') == null
        ? null
        : DateTime.fromMillisecondsSinceEpoch(row.read<int>('last_review')),
    reps: _int(row, 'reps'),
    lapses: _int(row, 'lapses'),
  );
}
